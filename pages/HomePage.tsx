...
import { sermons, events, ministries } from '../data/mockData';
import { ChevronRightIcon } from '../components/icons';
import Reveal from '../components/Reveal';

const HeroSection: React.FC = () => (
  <div className="relative h-screen bg-cover bg-center" style={{ backgroundImage: "url('/artifacts/sunday_service_worship_1763537430650.png')" }}>
  ...
  <section className="py-24 bg-white">
    <div className="container mx-auto px-4 sm:px-6 lg:px-8">
      <Reveal className="max-w-4xl mx-auto text-center">
        <h2 className="text-4xl md:text-5xl font-serif font-bold text-brand-dark-blue mb-6">Welcome Home</h2>
    ...
    <section className="py-24 bg-gradient-to-br from-brand-cream to-white">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <Reveal variant="right" className="bg-gray-300 rounded-lg shadow-xl overflow-hidden">
            <img
          ...
          <Reveal>
            ...
            <h2 className="text-4xl md:text-5xl font-serif font-bold text-brand-dark-blue mb-6 leading-tight">{latestSermon.title}</h2>
    ...
    <section className="py-24 bg-brand-dark-blue text-white">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <Reveal className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-serif font-bold mb-4">Join Us</h2>
          <div className="w-24 h-1 bg-brand-gold mx-auto"></div>
        </Reveal>
        <div className="grid md:grid-cols-3 gap-8">
          {ministries.map((ministry, index) => (
            <Reveal key={ministry.id} delay={index * 120}>
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl overflow-hidden text-center hover:bg-white/20 transition-all duration-300 transform hover:-translate-y-2 border border-white/10 h-full flex flex-col">
                <div className="h-40 overflow-hidden">
                  <img src={ministry.imageUrl} alt={ministry.name} className="w-full h-full object-cover" />
                </div>
                <div className="p-8 flex flex-col flex-1">
                  <h3 className="text-2xl font-serif font-bold mb-3">{ministry.name}</h3>
                  <p className="text-brand-gold font-semibold mb-4 tracking-wide">{ministry.meetingInfo}</p>
...
const UpcomingEvents: React.FC = () => (
  <section className="py-24 bg-white">
    <div className="container mx-auto px-4 sm:px-6 lg:px-8">
      <Reveal className="text-center mb-16">
        <h2 className="text-4xl md:text-5xl font-serif font-bold text-brand-dark-blue mb-4">Upcoming Events</h2>
        <div className="w-24 h-1 bg-brand-gold mx-auto"></div>
      </Reveal>
      <div className="grid md:grid-cols-3 gap-8">
        {events.slice(0, 3).map((event, index) => (
          <Reveal key={event.id} delay={index * 120}>
            <div className="bg-white rounded-2xl shadow-soft overflow-hidden group hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 h-full">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid md:grid-cols-3 gap-8">
          {actions.map((action, index) => (
            <Reveal key={action.title} delay={index * 120}>
              <Link
                to={action.link}
                className="group block p-10 bg-white rounded-2xl shadow-soft hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-3 border-t-4 border-brand-gold h-full"
              >
