    ...
    <div className="bg-white">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex flex-col md:flex-row gap-4 mb-8 p-4 bg-gray-100 rounded-lg">
          <select value={speakerFilter} onChange={e => setSpeakerFilter(e.target.value)} className="w-full md:w-1/2 p-2 border rounded-md">
            <option value="">All Speakers</option>
            {uniqueSpeakers.map(speaker => <option key={speaker} value={speaker}>{speaker}</option>)}
          </select>
          <select value={seriesFilter} onChange={e => setSeriesFilter(e.target.value)} className="w-full md:w-1/2 p-2 border rounded-md">
